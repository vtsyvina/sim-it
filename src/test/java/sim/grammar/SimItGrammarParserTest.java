...
public class SimItGrammarParserTest {
    ...
    public void testArithmeticCalculations(){
        Simulation simulation = parser.parseToSimulation(fileToString("src/test/resources/arithmetic.txt"));
        simulation.run();
    ...
    public void testTime(){
        Simulation simulation = parser.parseToSimulation(fileToString("src/test/resources/time.txt"));
        simulation.run();
    ...
    @Test
    public void ifTest(){
        Simulation simulation = parser.parseToSimulation(fileToString("src/test/resources/if.txt"));
        simulation.run();
        assertEquals(1, simulation.getContext().getEnvironment().get("it",1), 1e-5);
        assertEquals(10, simulation.getContext().getEnvironment().get("it",10), 1e-5);
